...
function getCart(listeProduits){
if (listeProduits === null || listeProduits == 0) {
...
// Insertion de "p" supprimer
    ...
    for (var i = 0; i < myLength; ++i) { // Boucle for qui compte et rajoute le prix du panier 
        totalPrice += (elemsQtt[i].valueAsNumber * produits[i].selectedPrice);  
    }
    ...
    let quantityModif = produits[k].selectedQuantity;
    console.log("quantityModif",quantityModif)
    
    let qttModifValue = qttModif[k].valueAsNumber;
    console.log("qttModifValue",qttModifValue)
    produits[k].selectedQuantity = qttModifValue
...
// Suppression d'un produit
 ...
 //Selection de l'element à supprimer en fonction de son id ET sa couleur
    let idDelete = produits[j].selectedId;
    let colorDelete = produits[j].selectedColors;
    listeProduits = produits.filter( el => el.selectedId !== idDelete || el.selectedColors !== colorDelete );
    console.log("listeProduits",listeProduits)       
    localStorage.setItem("panier", JSON.stringify(listeProduits));
